  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="relative flex items-center justify-center w-full flex-1 py-12 px-8 text-center z-30">
        ...
        <section className="relative flex flex-col md:flex-row flex-shrink-0 max-w-[1080px] w-full md:h-[640px] bg-white rounded-xl shadow-high overflow-hidden z-20">
          {data ? (
            <>
              <aside className={`${paymentFormIsVisible ? 'translate-y-[-85px] md:translate-y-0' : 'bg-gray-50 md:bg-transparent'} relative flex items-center justify-center md:w-2/5 xl:w-1/2 h-[250px] md:h-full md:overflow-hidden z-10 transition duration-500 ease-in-out`}>
                <img className={`${paymentFormIsVisible && 'md:translate-x-[-33%]'} scale-50 translate-y-[-40px] md:scale-100 max-w-min transition duration-500 ease-in-out`} src={`/images${data.image}`} width={800} height={800} />
              </aside>
              
              <article className={`${paymentFormIsVisible ? 'opacity-0 translate-x-[20px]' : 'opacity-100'} relative flex flex-col justify-between w-full md:w-3/5 xl:w-1/2 p-6 space-y-4 md:space-y-0 text-left md:bg-gradient-to-r from-gray-50 to-white shadow-crease md:shadow-fold transition duration-500 ease-in-out z-20`}>
              
                <div className="flex flex-col md:p-8 md:pt-4 space-y-2">
                  <h1 className="font-serif text-4xl sm:text-5xl leading-none sm:leading-tight capitalize sm:truncate">{data.name}</h1>
                  <p className="text-black/75 text-sm sm:text-base">{data.teaser}</p>
                </div>
                
                <div className="flex flex-col md:px-8">
                  <dl className="grid sm:grid-cols-3">
                    <dt className="text-black/75 text-sm sm:border-b border-gray-200 sm:py-2 pt-2 sm:pt-3">Collection</dt>
                    <dd className="font-serif text-lg col-span-2 border-b border-gray-200 py-2 pt-0 sm:pt-2">Foundations</dd>
                    <dt className="text-black/75 text-sm sm:border-b border-gray-200 sm:py-2 pt-4 sm:pt-3">Origin</dt>
                    <dd className="font-serif text-lg col-span-2 border-b border-gray-200 py-2 pt-0 sm:pt-2">Summer 2014</dd>
                    <dt className="text-black/75 text-sm sm:py-2 pt-4 sm:pt-3">Ingredients</dt>
                    <dd className="font-serif col-span-2 py-2 pt-2.5">
            ...
            <div className="flex justify-center items-center w-full h-full">
              <span className="animate-ping w-3 h-3 rounded-full bg-gray-200"></span>
            </div>
          )}
        </section>
