        ...
        <li className="flex flex-shrink-0 overflow-visible">
          <Link href={`/coffee/${props.coffee.id}`} scroll={false}>
            <a className="relative flex flex-col items-center group">
              ...
              <span className="absolute left-1/2 bottom-2 translate-x-[-50%] translate-y-[4px] group-hover:translate-y-0 whitespace-nowrap text-black/75 uppercase text-[10px] tracking-widest text-center bg-gray-50 backdrop-blur-md py-0.5 px-2 shadow-high rounded-full opacity-0 group-hover:opacity-100 transition ease-in-out duration-500 hover:delay-500">{`${props.coffee.name}`}</span>
